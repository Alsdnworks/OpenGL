...
void Context::Render()
{
  ...
  glViewport(0, 0, m_width, m_height);
  m_ssaoProgram->Use();
  glActiveTexture(GL_TEXTURE0);                                          //텍스쳐헷갈릴때 14-4 3620
...
///
  ...
  for (size_t i = 0; i < m_deferLights.size(); i++)
  {
    ...
    auto colorName = fmt::format("lights[{}].color", i); //spdlog기능 14-3 1225
...
//라이트박스 구현체
  ...
  m_lightingShadowProgram->SetUniform("light.directional",m_light.directional ? 1:0);//트루면 1 아님 0
